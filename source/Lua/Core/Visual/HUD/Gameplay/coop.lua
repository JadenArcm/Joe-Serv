...
local function drawScore(v, player)
	...
	if (alpha ~= false) then
		v.drawScaled(anim, y, FRACUNIT, v.cachePatch(patch), flags | alpha, v.getColormap(TC_DEFAULT, color))
		joeFuncs.drawNum(v, anim + (22 * FRACUNIT), y + (3 * FRACUNIT), player.score, flags | alpha, {font = "JOE_BNUM", space = 7})
	end
end
...
local function drawRings(v, player)
	...
	if (alpha ~= false) then
		v.drawScaled(anim, y, FRACUNIT, v.cachePatch(patch), flags | alpha, color)
		joeFuncs.drawNum(v, anim - (8 * FRACUNIT), y + (3 * FRACUNIT), player.rings, flags | alpha, {font = "JOE_BNUM", space = 7, align = "right"})
	end
end
...
local function drawTime(v, player)
	...
	if (alpha ~= false) then
		...
		if (style.value < 3) then
			joeFuncs.drawNum(v, anim - ((51 + xoffs) * FRACUNIT), y, G_TicsToMinutes(time.tics, true), flags | alpha, {font = "JOE_BNUM", space = 7, align = "right"})

			v.drawScaled(anim - ((52 + xoffs) * FRACUNIT), y, FRACUNIT, v.cachePatch("JOE_COLON"), flags | alpha, nil)
			joeFuncs.drawNum(v, anim - ((30 + xoffs) * FRACUNIT), y, G_TicsToSeconds(time.tics), flags | alpha, {font = "JOE_BNUM", space = 7, pad = 2, align = "right"})

			if (style.value > 0) then
				v.drawScaled(anim - (30  * FRACUNIT), y, FRACUNIT, v.cachePatch("JOE_PERIO"), flags | alpha, nil)
				joeFuncs.drawNum(v, anim - (8 * FRACUNIT), y, G_TicsToCentiseconds(time.tics), flags | alpha, {font = "JOE_BNUM", space = 7, pad = 2, align = "right"})
			end
		else
			joeFuncs.drawNum(v, anim - (8 * FRACUNIT), y, time.tics, flags | alpha, {font = "JOE_BNUM", space = 7, align = "right"})
		end
	end
end
