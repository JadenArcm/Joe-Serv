...
joeFuncs.addHUD(function(v, player)
	...
	searchBlockmap("objects", function(_, mo)
		...
		local alpha = ((joeFuncs.getDistance(mo, player.realmo) >= (maxdist / 2)) or not P_CheckSight(player.realmo, mo)) and V_60TRANS or V_40TRANS
		local info = (mo.health * 100) / mo.info.spawnhealth
		...
		if (screen.visible) then
			...
			joeFuncs.drawFill(v, screen.x - ((bar_width - bar_offs) / 2), screen.y + (8 * FRACUNIT), bar_width - bar_offs, 6 * FRACUNIT, 28 | (alpha - V_20TRANS))
			...
			v.drawString(screen.x - (28 * FRACUNIT), screen.y + (9 * FRACUNIT), info .. "%", 0, "small-fixed")
			v.drawString(screen.x + (28 * FRACUNIT), screen.y + (9 * FRACUNIT), mo.health .. " / " .. mo.info.spawnhealth, 0, "small-fixed-right")
		end
