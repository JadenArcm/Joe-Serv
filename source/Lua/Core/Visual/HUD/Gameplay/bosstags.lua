...
joeFuncs.addHUD(function(v, player)
	...
	searchBlockmap("objects", function(_, mo)
		...
		local alpha = ((joeFuncs.getDistance(mo, player.realmo) >= (maxdist / 2)) or not P_CheckSight(player.realmo, mo)) and V_60TRANS or V_30TRANS
		...
		if (screen.visible) then
			...
			v.drawString(screen.x - (28 * FRACUNIT), screen.y + (9 * FRACUNIT), info .. "\x86%", (alpha - V_30TRANS), "small-fixed")
